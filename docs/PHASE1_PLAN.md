...
**Status:** Tranche A (audit §7 items 1–2) is merged and live
(`4d775a181`, PR #6). The HLC gate hotfix is live (`d57bff8d2`).
Tranches B, C, D below are **designed but NOT approved** — each tranche
requires explicit editor sign-off before any code is written, and
approving one tranche approves only that tranche.
...
- **Accepted caveat (already signed off in principle, restate in the
  ...
  loud in the PR body.
- Note: `refresh-ipeds-full.yml` also pushes occasionally. Check whether
  it belongs under the same umbrella while in there; if yes, propose it
  as an explicit option in the PR rather than silently including it.
...
2. **One-directionalize `notice↔monitoring`:** a new `notice` candidate
   ...
   allowed.
3. **Never fold text into an approved row silently.** If the matched
   override row is `approved` and the incoming text differs beyond the
   similarity threshold, either (a) skip the fold and log, or (b) fold
   and flip `review_status` to `unreviewed` so it re-enters the queue.
   **(a) vs (b) is an editorial-policy choice — ask the editor before
   implementing.** (b) is safer for correctness but re-queues work.
...
**Fix direction (pick one, propose to editor before building):**

- **(preferred per audit)** persist the text-derived classification into
  `dapip_action_rows_filtered.csv` at DAPIP-build time
  (`build_dapip_accreditation_actions.R`), so every downstream consumer
  reads the same committed value and the cache is consulted exactly
  once, in one place; or
- classify from committed CSV fields only (drop the cache read at
  candidates/export time).
...
### Editorial-policy question parked under C (from §3.10)

Whether merger/absorption approvals (the VWU/Sentara class — DAPIP
`dapip_backed_keep` rows classified into the monitoring/procedural
family by PDF text) *should* surface on the public site is an editorial
question, not a defect. Ask the editor when C3 is picked up; the
pipeline currently excludes them consistently.
...
**Also fold in (from Tranche A review notes):** the accreditation side
has no stale-row dropper at all (cuts-only concept today). If the
unification makes adding one natural, propose it separately to the
editor — it is a behavior change, not simplification.
...
**Direction:** in `build_dapip_vs_scraper_audit.R`, when
`dapip_action_rows_raw.csv` is absent, skip the coverage-CSV write
entirely (leave the committed copy untouched) instead of writing a
degraded one; log one line saying so. Alternatively remove
`dapip_code_coverage.csv` from publish's add list — weaker (the file
still gets locally overwritten) but zero-code. Prefer the skip.
...
| 3 | C1 fail-closed matcher | M | none; needs editor answer on fold-vs-flip |
| 4 | C2 tombstones | L | ship alone; touches every override consumer |
| 5 | C3 determinism | M | none; needs editor answer on fix direction + the merger-visibility policy question |
