...
# Transforms raw IPEDS data into the canonical dataset: cleaned, decoded, and enriched
# with derived metrics. Outputs two CSVs - canonical (website/workbook) and extended
# (analysis fields) - from the raw institution-year data.
# ====== BUILD CANONICAL IPEDS DATASET =============================================
...
main <- function(cli_args = NULL) {
  ...
  # ---------------------------------------------------------------------------
  # HELPER: Convert year-keyed auxiliary table list to long format
  ...
  # ---------------------------------------------------------------------------
  # HELPER: Load research expense and core expense data for a given year
  ...
  # Use latest institution name/city/state for consistent display
  ...
  # Keep backfill table for filling gaps in older source files
  ...
  # Backfill gaps in enrollment/staff fields from auxiliary EFFY and EAP tables
  ...
  # Filter to 2024 cohort institutions
