main <- function(cli_args = NULL) {
  source(file.path(getwd(), "scripts", "shared", "utils.R"))
  args <- parse_cli_args(cli_args)
  get_arg_value <- function(flag, default = NULL) get_arg(args, flag, default)
  has_flag <- function(flag) arg_has(args, flag)

  ensure_packages(c("readr", "googlesheets4"))
  source(file.path(getwd(), "scripts", "shared", "editorial_review_helpers.R"))
  source(file.path(getwd(), "scripts", "shared", "google_sheets_helpers.R"))

  candidates_path <- get_arg_value(
    "--candidates",
    file.path(getwd(), "data_pipelines", "college_cuts", "closure_flags_review_candidates.csv")
  )
  default_sheet <- Sys.getenv(
    "COLLEGE_CUTS_REVIEW_SHEET_ID",
    unset = Sys.getenv("ACCREDITATION_REVIEW_SHEET_ID", unset = NA_character_)
  )
  sheet_id_or_url <- get_arg_value("--sheet", default_sheet)
  sheet_tab <- get_arg_value(
    "--tab",
    Sys.getenv("COLLEGE_CUTS_CLOSURE_FLAGS_REVIEW_SHEET_TAB", unset = "closure_flags_review")
  )
  auth_json <- get_arg_value("--auth-json", Sys.getenv("GOOGLE_APPLICATION_CREDENTIALS", unset = NA_character_))
  email <- get_arg_value("--email", NA_character_)
  cache_dir <- get_arg_value("--cache", file.path(getwd(), ".secrets", "googlesheets4"))
  verbose <- has_flag("--verbose")

  sheet_target <- trimws(as.character(sheet_id_or_url %||% ""))
  if (!nzchar(sheet_target)) {
    stop("Provide --sheet <Google Sheet URL or ID> or set the cuts review sheet id environment variable.", call. = FALSE)
  }

  if (!file.exists(candidates_path)) {
    message("No closure flags review candidates at ", candidates_path, " - skipping closure flags sheet append.")
    return(invisible(list(appended = 0L, tab = sheet_tab)))
  }

  candidates <- coerce_college_cuts_closure_flags_review_sheet_rows(
    readr::read_csv(
      candidates_path,
      col_types = readr::cols(.default = readr::col_character()),
      show_col_types = FALSE
    )
  )

  authenticate_google_sheets(
    auth_json = auth_json,
    email = email,
    cache_dir = cache_dir,
    scopes = "spreadsheets",
    verbose = verbose
  )

  sheet_target <- extract_google_sheet_id(sheet_target)
  tab_exists <- google_sheet_tab_exists(sheet_target, sheet_tab)
  sheet_rows <- if (tab_exists) {
    read_google_sheet_table(
      ss = sheet_target,
      sheet_name = sheet_tab,
      verbose = verbose
    )
  } else {
    data.frame(stringsAsFactors = FALSE)
  }

  appended <- 0L
  if (!tab_exists || !nrow(sheet_rows)) {
    if (!tab_exists) {
      ensure_google_sheet_tab(sheet_target, sheet_tab)
    }
    if (verbose) {
      message("Writing initial closure flags review tab: ", sheet_tab)
    }
    googlesheets4::sheet_write(
      data = candidates[, COLLEGE_CUTS_CLOSURE_FLAGS_REVIEW_SHEET_COLUMNS, drop = FALSE],
      ss = sheet_target,
      sheet = sheet_tab
    )
    appended <- nrow(candidates)
  } else {
    assert_college_cuts_closure_flags_review_sheet_header(sheet_rows)
    assert_review_sheet_header_order(
      sheet_rows,
      COLLEGE_CUTS_CLOSURE_FLAGS_REVIEW_SHEET_COLUMNS,
      sheet_tab
    )
    append_rows <- build_college_cuts_closure_flags_review_sheet_append_rows(
      candidates,
      sheet_rows
    )
    if (nrow(append_rows)) {
      googlesheets4::sheet_append(
        ss = sheet_target,
        data = append_rows[, COLLEGE_CUTS_CLOSURE_FLAGS_REVIEW_SHEET_COLUMNS, drop = FALSE],
        sheet = sheet_tab
      )
    }
    appended <- nrow(append_rows)
  }

  message(sprintf("closure flags review sheet rows appended: %d (tab: %s)", appended, sheet_tab))
  invisible(list(appended = appended, tab = sheet_tab))
}

if (sys.nframe() == 0) {
  main()
}
