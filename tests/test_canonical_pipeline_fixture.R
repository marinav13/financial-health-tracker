...
run_test("IPEDS canonical pipeline fixture fails clearly when dictionary decoding filters every row", function() {
  fixture_root <- tempfile("canonical-empty-prepared-")
  dir.create(fixture_root, recursive = TRUE, showWarnings = FALSE)
  on.exit(unlink(fixture_root, recursive = TRUE, force = TRUE), add = TRUE)
  old_wd <- getwd()
  on.exit(setwd(old_wd), add = TRUE)
  ...
  assert_true(!is.null(err), "Expected canonical build to fail when dictionary decoding filters every row.")
  assert_true(grepl("Canonical row build produced an unusable prepared_rows table", err, fixed = TRUE))
  assert_true(grepl("raw_enriched rows=2; prepared_rows rows=0", err, fixed = TRUE))
  assert_true(grepl("level-four-years=0/2", err, fixed = TRUE))
})
